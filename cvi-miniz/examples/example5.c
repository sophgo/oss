// example5.c - Demonstrates how to use cvi_miniz.c's low-level tdefl_compress() and tinfl_inflate() API's for simple file to file compression/decompression.
// The low-level API's are the fastest, make no use of dynamic memory allocation, and are the most flexible functions exposed by cvi_miniz.c.
// Public domain, April 11 2012, Rich Geldreich, richgel99@gmail.com. See "unlicense" statement at the end of tinfl.c.
// For simplicity, this example is limited to files smaller than 4GB, but this is not a limitation of cvi_miniz.c.
...
// Now include stdio.h because this test uses fopen(), etc. (but we still don't want cvi_miniz.c's stdio stuff, for testing).
#include <stdio.h>
#include <limits.h>
...
int main(int argc, char *argv[])
{
   ...
   printf("cvi_miniz.c example5 (demonstrates tinfl/tdefl)\n");
   ...
   {
      // This is not a limitation of cvi_miniz or tinfl, but this example.
