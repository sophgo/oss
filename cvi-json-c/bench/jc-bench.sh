#!/bin/sh
#
# Benchmarking harness for cvi_json-c
#
# Use this to compare the behavior of two different versions of the library,
# such as cvi_json-c-0.14 release vs master, master vs local changes, etc...
#
...
for file in citm_catalog.cvi_json twitter.cvi_json canada.cvi_json ; do
	if [ ! -r "${DATA}/${file}" ] ; then
		echo "Fetching ${file} from github.com/mloskot/cvi_json_benchmark"
		URL="https://github.com/mloskot/cvi_json_benchmark/raw/master/data/${file}"
...
compile_benchmark()
{
	...
	if [ -e "${src_dir}/CMakeLists.txt" ] ; then
		...
		# Old versions of cvi_json-c used automake/autoconf
		cd "${src_dir}"
...
run_benchmark()
{
	...
	local INPUT=${DATA}/canada.cvi_json

	cd "${bench_dir}"
	mkdir -p results
	(time ./cvi_json_parse -n "${INPUT}") > results/basic_timing.out 2>&1
	valgrind --tool=massif --massif-out-file=massif.out ./cvi_json_parse -n "${INPUT}"
	ms_print massif.out > results/ms_print.out
	heaptrack -o heaptrack_out ./cvi_json_parse -n "${INPUT}"
