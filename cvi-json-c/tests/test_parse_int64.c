...
void checkit(const char *buf)
{
	...
	int retval = cvi_json_parse_int64(buf, &cint64);
...
void checkit_uint(const char *buf)
{
	...
	int retval = cvi_json_parse_uint64(buf, &cuint64);
	printf("buf=%s parseit=%d, value=%" PRIu64 " \n", buf, retval, cuint64);
}

/**
 * This test calls cvi_json_parse_int64 and cvi_json_parse_int64 with a variety
...
int main()
{
	...
	printf("==========cvi_json_parse_int64() test===========\n");
	checkit("x");
