...
int main(int argc, char **argv)
{
	 ...
	 */
	cvi_json_object *my_object = cvi_json_object_new_object();
	cvi_json_object_object_add(my_object, "foo1", cvi_json_object_new_string("bar1"));
	cvi_json_object_object_add(my_object, "foo2", cvi_json_object_new_string("bar2"));
	cvi_json_object_object_add(my_object, "deleteme", cvi_json_object_new_string("bar2"));
	cvi_json_object_object_add(my_object, "foo3", cvi_json_object_new_string("bar3"));

	printf("==== delete-in-loop test starting ====\n");

	int orig_count = 0;
	cvi_json_object_object_foreach(my_object, key0, val0)
	{
		printf("Key at index %d is [%s] %d", orig_count, key0, (val0 == NULL));
		if (strcmp(key0, "deleteme") == 0)
		{
			cvi_json_object_object_del(my_object, key0);
			printf(" (deleted)\n");
		}
		else
			printf(" (kept)\n");
	...
	cvi_json_object_object_foreach(my_object, key, val)
	{
		...
		original_key = key;
		cvi_json_object_object_add(my_object, key, cvi_json_object_new_string("zzz"));
	}
	...
	int new_count = 0;
	int retval = 0;
	cvi_json_object_object_foreach(my_object, key2, val2)
	{
