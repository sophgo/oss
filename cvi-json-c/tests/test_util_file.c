...
static void test_write_to_file()
{
	...
	jso = cvi_json_tokener_parse("{"
	                         ...
	                         "}");
	const char *outfile = "cvi_json.out";
	int rv = cvi_json_object_to_file(outfile, jso);
	printf("%s: cvi_json_object_to_file(%s, jso)=%d\n", (rv == 0) ? "OK" : "FAIL", outfile, rv);
	if (rv == 0)
		stat_and_cat(outfile);

	putchar('\n');

	const char *outfile2 = "cvi_json2.out";
	rv = cvi_json_object_to_file_ext(outfile2, jso, JSON_C_TO_STRING_PRETTY);
	printf("%s: cvi_json_object_to_file_ext(%s, jso, JSON_C_TO_STRING_PRETTY)=%d\n",
	       (rv == 0) ? "OK" : "FAIL", outfile2, rv);
	if (rv == 0)
		stat_and_cat(outfile2);

	const char *outfile3 = "cvi_json3.out";
	...
	rv = cvi_json_object_to_fd(d, jso, JSON_C_TO_STRING_PRETTY);
	printf("%s: cvi_json_object_to_fd(%s, jso, JSON_C_TO_STRING_PRETTY)=%d\n",
	       (rv == 0) ? "OK" : "FAIL", outfile3, rv);
	// Write the same object twice
	rv = cvi_json_object_to_fd(d, jso, JSON_C_TO_STRING_PLAIN);
	printf("%s: cvi_json_object_to_fd(%s, jso, JSON_C_TO_STRING_PLAIN)=%d\n",
	       (rv == 0) ? "OK" : "FAIL", outfile3, rv);
	close(d);
	if (rv == 0)
...
{
	//	cvi_json_object_to_file(file, obj);
	//	cvi_json_object_to_file_ext(file, obj, flags);
	...
	//	Test cvi_json_c_version.c
	if (strncmp(cvi_json_c_version(), JSON_C_VERSION, sizeof(JSON_C_VERSION)))
	{
		printf("FAIL: Output from cvi_json_c_version(): %s "
		       "does not match %s",
		       cvi_json_c_version(), JSON_C_VERSION);
		return EXIT_FAILURE;
	}
	if (cvi_json_c_version_num() != JSON_C_VERSION_NUM)
	{
		printf("FAIL: Output from cvi_json_c_version_num(): %d "
		       "does not match %d",
		       cvi_json_c_version_num(), JSON_C_VERSION_NUM);
		return EXIT_FAILURE;
	}
...
static void test_read_valid_with_fd(const char *testdir)
{
	...
	cvi_json_object *jso = cvi_json_object_from_fd(d);
	if (jso != NULL)
	{
		printf("OK: cvi_json_object_from_fd(valid.json)=%s\n", cvi_json_object_to_cvi_json_string(jso));
		cvi_json_object_put(jso);
	}
	else
	{
		fprintf(stderr, "FAIL: unable to parse contents of %s: %s\n", filename,
		        cvi_json_util_get_last_err());
	}
	close(d);
}
...
static void test_read_valid_nested_with_fd(const char *testdir)
{
	...
	assert(NULL == cvi_json_object_from_fd_ex(d, -2));
	cvi_json_object *jso = cvi_json_object_from_fd_ex(d, 20);
	if (jso != NULL)
	{
		printf("OK: cvi_json_object_from_fd_ex(valid_nested.json, 20)=%s\n",
		       cvi_json_object_to_cvi_json_string(jso));
		cvi_json_object_put(jso);
	}
	else
	...
	jso = cvi_json_object_from_fd_ex(d, 3);
	if (jso != NULL)
	{
		printf("FAIL: cvi_json_object_from_fd_ex(%s, 3)=%s\n", filename,
		       cvi_json_object_to_cvi_json_string(jso));
		cvi_json_object_put(jso);
	}
	else
...
{
	const char *filename = "./not_present.cvi_json";

	cvi_json_object *jso = cvi_json_object_from_file(filename);
	if (jso != NULL)
	{
		printf("FAIL: cvi_json_object_from_file(%s) returned %p when NULL expected\n", filename,
		       (void *)jso);
		cvi_json_object_put(jso);
	}
	else
	{
		printf("OK: cvi_json_object_from_file(%s) correctly returned NULL: %s\n", filename,
		       cvi_json_util_get_last_err());
	}
}
...
static void test_read_closed()
{
	...
	cvi_json_object *jso = cvi_json_object_from_fd(fixed_d);
	if (jso != NULL)
	{
	...
	printf("OK: cvi_json_object_from_fd(closed_fd), "
...
static void test_read_fd_equal(const char *testdir)
{
	...
	cvi_json_object *new_jso = cvi_json_object_from_fd(d);
	close(d);

	printf("OK: cvi_json_object_from_file(valid.json)=%s\n", cvi_json_object_to_cvi_json_string(jso));
	printf("OK: cvi_json_object_from_fd(valid.json)=%s\n", cvi_json_object_to_cvi_json_string(new_jso));
	cvi_json_object_put(jso);
	cvi_json_object_put(new_jso);
}
