...
cvi_json_c_TOP := $(LOCAL_PATH)

JSON_C_BUILT_SOURCES := Android.mk

JSON_C_BUILT_SOURCES := $(patsubst %, $(abspath $(cvi_json_c_TOP))/%, $(JSON_C_BUILT_SOURCES))

.PHONY: cvi_json-c-configure cvi_json-c-configure-real
cvi_json-c-configure-real:
	echo $(JSON_C_BUILT_SOURCES)
	cd $(cvi_json_c_TOP) ; \
	$(abspath $(cvi_json_c_TOP))/autogen.sh && \
	CC="$(CONFIGURE_CC)" \
	...
	$(abspath $(cvi_json_c_TOP))/$(CONFIGURE) --host=$(CONFIGURE_HOST) \
	--prefix=/system \
	&& \
...
cvi_json-c-configure: cvi_json-c-configure-real

PA_CONFIGURE_TARGETS += cvi_json-c-configure

-include $(cvi_json_c_TOP)/Android.mk
